...
//        }
    ...
    private fun requestKakaoLogin() {
        val kakaoCallback: (OAuthToken?, Throwable?) -> Unit = { token, error ->
            if (error != null) {
                Toast.makeText(this@MainActivity, "카카오 아이디 로그인 실패!", Toast.LENGTH_SHORT).show()
            } else if (token != null) {
                UserApiClient.instance.me { user, err ->
                    if (err != null) return@me
                    Toast.makeText(this@MainActivity, "카카오 아이디 로그인 성공! 1 -> ${user?.id}", Toast.LENGTH_SHORT).show()
                    val loginObject = UserLoginInfo(
        ...
        if (UserApiClient.instance.isKakaoTalkLoginAvailable(this)) {
            UserApiClient.instance.loginWithKakaoTalk(this) { token, error ->
                if (error != null) {
                    ...
                    Toast.makeText(this@MainActivity, "카카오 아이디 로그인 성공! 2 -> ${token.accessToken}", Toast.LENGTH_SHORT).show()
                }
            }
        } else {
            UserApiClient.instance.loginWithKakaoAccount(this, callback = kakaoCallback)
